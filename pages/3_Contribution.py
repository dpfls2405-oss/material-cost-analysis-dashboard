...
with col1:
    st.plotly_chart(bar_contribution(top_up, "재료비율 상승 TOP 품목"), use_container_width=True)
with col2:
    st.plotly_chart(bar_contribution(top_down, "재료비율 하락 TOP 품목"), use_container_width=True)

current_ratio = monthly.loc[monthly["month"] == selected_month, "material_ratio"].iloc[0]
...
st.plotly_chart(waterfall_contribution(prev_ratio, wf, current_ratio), use_container_width=True)
